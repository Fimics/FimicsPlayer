...
import com.mic.router.api.core.RouterLoadPath;
...
public final class RouterManager {
    ...
    private LruCache<String, RouterLoadPath> pathCache;
    // APT生成的路由组Group源文件前缀名
    private static final String GROUP_FILE_PREFIX_NAME = ".Router$$Group$$";
    ...
    Object navigation(@NonNull Context context, BundleManager bundleManager, int code) {
        ...
        try {
            RouterLoadGroup groupLoad = groupCache.get(group);
            if (groupLoad == null) {
                Class<?> clazz = Class.forName(groupClassName);
                groupLoad = (RouterLoadGroup) clazz.newInstance();
            ...
            RouterLoadPath pathLoad = pathCache.get(path);
            if (pathLoad == null) {
